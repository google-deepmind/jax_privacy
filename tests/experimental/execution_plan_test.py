...
class ExecutionPlanTest(parameterized.TestCase):
  ...
  @parameterized.parameters(
      {"sampling_prob": None, "expected_batch_size": None},
      {"sampling_prob": 0.1, "expected_batch_size": 4},
      {"expected_batch_size": 4, "num_examples": None},
      {"truncated_batch_size": 4, "num_examples": None},
      {
          "sampling_prob": 0.1,
          "neighboring_relation": (
  ...
  def test_dpsgd_validation(self, **kwargs):
    default_kwargs = {
        "iterations": 10,
        "sampling_prob": 0.1,
        "epsilon": 1.0,
    ...
    default_kwargs.update(kwargs)
    with self.assertRaises(ValueError):
      execution_plan.DpsgdExecutionPlanConfig(**default_kwargs)

  @parameterized.parameters(
      {"sampling_prob": 0.1},
      {"expected_batch_size": 10, "num_examples": 100},
  )
  def test_dpsgd_execution_plan_creation(self, **sampling_kwargs):
    ...
    self.assertIsInstance(plan.dp_event, dp_accounting.DpEvent)
    self.assertLen(
        list(plan.batch_selection_strategy.batch_iterator(100)), 12
    )

  def test_dpsgd_expected_batch_size_sampling_prob(self):
    config = execution_plan.DpsgdExecutionPlanConfig(
        iterations=5,
        epsilon=1.0,
        delta=1e-6,
    ...
    gradient_fn = clipping.clipped_grad(jnp.mean, l2_clip_norm=1.0)
    plan = config.make(gradient_fn)
    # group_size = 100 // 2 = 50, sampling_prob = 10 / 50 = 0.2.
    self.assertAlmostEqual(
        plan.batch_selection_strategy.sampling_prob, 0.2, places=6
    )
