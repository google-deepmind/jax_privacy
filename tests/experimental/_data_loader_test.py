...
def _make_config(iterations, noise_multiplier=1.0, sampling_prob=1.0):
  config = execution_plan.BandMFConfig.default(
      num_bands=1,
      iterations=iterations,
      noise_multiplier=noise_multiplier,
      expected_participations=sampling_prob * iterations,
  )
  return config
...
class DPTrainerPygrainTest(parameterized.TestCase):
  ...
  def test_basic_training_completes(self):
    ...
    trainer = training.DPTrainer(
        config=config, loss_fn=_quadratic_loss, optimizer=optimizer
    )
    state = trainer.fit(ds, params, rng_or_seed=0)
  ...
  def test_params_change(self):
    ...
    ds = self._make_dataset(num_examples=4, dim=2)
    config = _make_config(iterations=5, noise_multiplier=0.0)
    optimizer = optax.sgd(0.1)

    trainer = training.DPTrainer(
        config=config, loss_fn=_quadratic_loss, optimizer=optimizer
    )
    state = trainer.fit(ds, params, rng_or_seed=42)
  ...
  def test_in_memory_path_still_works(self):
    ...
    trainer = training.DPTrainer(
        config=config, loss_fn=loss_fn, optimizer=optimizer
    )
