...
class DPTrainerTest(parameterized.TestCase):
  ...
  def test_get_batch_map_dataset_missing_padding_example_raises(self):
    """Test _get_batch raises ValueError if padding_example is missing."""
    grain_data = [{'x': np.array([1.0])}]
    grain_ds = grain.MapDataset.source(grain_data)
    indices = np.array([0, -1], dtype=np.int32)
    with self.assertRaises(ValueError):
      training._get_batch(grain_ds, indices, padding_example=None)
