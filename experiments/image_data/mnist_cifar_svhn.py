...
def _tfds_load_raw_data(
    data_loader: '_DataLoader',
    *,
    shuffle_files: bool,
) -> tf.data.Dataset:
  ds = tfds.load(
      name=data_loader.config.name,
...
def _pygrain_load_raw_data(
  ...
  ds = pygrain.MapDataset.source(source)
  return ds.map(base.DataInputs.from_dict)
...
class _DataLoader(loader.DataLoader):
  ...
  load_raw_data_tf_fn: loader.LoadRawDataTfFn[Self] = _tfds_load_raw_data
  load_raw_data_grain_fn: loader.LoadRawDataGrainFn[Self] = (
      _pygrain_load_raw_data
  )
