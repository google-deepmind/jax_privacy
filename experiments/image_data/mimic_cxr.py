...
class MimicCxrConfig(base.ImageDatasetConfig):
  ...
  def _preprocess_label(self, label: base.TensorOrArray) -> base.TensorOrArray:
    if isinstance(label, tf.Tensor):
      ...
      label = np.reshape(label, [len(self.all_label_names)])
      label = np.astype(label, np.int32)
      # Sub-selects only the required classes.
      label = label[self.indices_in_select_label_order]
      # -1=uncertain/not mentioned, 0=negative, 1=positive
      return np.maximum(label, 0)
...
class AbstractMimicCxrLoader(loader.DataLoader, metaclass=abc.ABCMeta):
  ...
  def keep_example(self, key: tf.Tensor, record: tf.Tensor) -> tf.Tensor:
