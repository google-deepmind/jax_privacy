...
class MultiClassForwardFn(
  ...
  def train_forward(
    ...
    # Metadata can provide weights for the loss of each example.
    weights = inputs.metadata.get('weights', jnp.ones(images.shape[0]))
    ...
    metrics = typing.Metrics(
        ...
        per_example={'loss': loss, 'weighted_loss': loss * weights},
    )
    return jnp.mean(loss * weights), (network_state, metrics)
