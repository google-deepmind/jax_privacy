...
def build_train_input_dataset(
  ...
  if image_size_train == (32, 32):
    ds = tfds.load(
        name='imagenet_resized/32x32',
        split=dataset.train.split_content,
        as_supervised=True,
    )

    preprocess_fn = functools.partial(
        image_dataset_loader.preprocess_32x32,
        is_training=True,
        normalization_fn=lambda images: 2*images-1,
        augmult=augmult,
