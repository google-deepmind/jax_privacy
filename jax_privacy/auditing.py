...
class BootstrapParams:
  ...
  Optionally performs bias and acceleration correction for the bootstrap [B.
  Efron and R. J. Tibshirani, "An Introduction to the Bootstrap"]. Be aware that
  acceleration requires jackknife resampling, which computes the function n
  times for n total canaries.
  ...
  Attributes:
    ...
    bias_correction: Whether to correct for bias in the bootstrap resamples.
...
class CanaryScoreAuditor:
  ...
  def _bootstrap(
    ...
    # Use Laplace smoothing here to avoid division by zero.
    ...
    if params.acceleration:
      ...
      for i in range(len(in_scores)):
        jackknife_estimates.append(
            fn(CanaryScoreAuditor(np.delete(in_scores, i), out_scores))
        )
      for i in range(len(out_scores)):
        jackknife_estimates.append(
            fn(CanaryScoreAuditor(in_scores, np.delete(out_scores, i)))
        )
