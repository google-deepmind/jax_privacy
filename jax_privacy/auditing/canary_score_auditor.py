...
class CanaryScoreAuditor:
  ...
  def epsilon_one_shot(
      self, alpha: float, delta: float, one_sided: bool = True
  ) -> float:
    ...
    p = alpha / len(self._fn_counts)

    def get_eps(n_guess: int, n_correct: int) -> float:
      eps_min = 0
    ...
    for tp, fp in zip(tp_counts, fp_counts):
      ...
      if n_correct / n_guess <= required_q:
        ...
        # to contain only points on the convex hull. If the mean is worse than
        # the best epsilon we've seen so far, we can stop.
        break
      ...
      best_eps = max(best_eps, get_eps(n_guess, n_correct))
