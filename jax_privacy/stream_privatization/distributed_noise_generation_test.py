...
class ShardedNoiseGenerationTest(parameterized.TestCase):
  ...
  def test_raises_with_positional_shardings(self):
    C1 = toeplitz.inverse_as_streaming_matrix(
        jnp.array([1, 0.5, 0.3, 0.25]), column_normalize_for_n=None
    )
    out_sharding = jax.sharding.PositionalSharding(jax.devices())

    with self.assertRaisesRegex(ValueError, 'NamedSharding'):
  ...
  def test_reshape_add(self):
    ...
    mesh = jax.sharding.Mesh(
        np.array(jax.devices()).reshape(2, 2, 2), ('x', 'y', 'z')  # pylint: disable=too-many-function-args
    )
    sharding = jax.sharding.NamedSharding(
        mesh, jax.sharding.PartitionSpec(None, 'y', 'x')
    )

    z = jax.jit(distributed_noise_generation._reshape_add, static_argnums=2)(  # pylint: disable=not-callable
        x, y, sharding
    )
    self.assertEqual(z.shape, x.shape)
    self.assertEqual(z.sharding, sharding)
    chex.assert_trees_all_equal(jnp.sort(z.flatten()), y)

  @parameterized.named_parameters(
      ('default_internal_flat_shardings', lambda _: None),
      ('custom_internal_flat_shardings', flatten_shardings_fn),
      ('empty_internal_flat_shardings', empty_shardings_fn),
      ('blt', lambda _: None, buffered_toeplitz_noising_matrix_fn),
      ('dpsgd', lambda _: None, streaming_matrix.identity),
      ('prefix', lambda _: None, streaming_matrix.prefix_sum),
      ('momentum', lambda _: None, streaming_matrix.momentum_sgd_matrix),
      {'testcase_name': 'no_out_sharding', 'provide_out_sharding': False},
  )
  def test_runs_as_expected(
      self,
      internal_shardings_fn=lambda _: None,
      strategy_inverse_fn=banded_toeplitz_noising_matrix_fn,
      provide_out_sharding=True,
  ):
    self.assertEqual(jax.device_count(), 8)

    mesh = jax.sharding.Mesh(mesh_utils.create_device_mesh((4, 2)), ('x', 'y'))
    pspecs = {
    ...
    internal_flat_shardings = internal_shardings_fn(out_sharding)
    ...
    model_params = jax.tree.map(jax.device_put, model_params, out_sharding)

    privatizer = (
        distributed_noise_generation.streaming_matrix_to_sharded_privatizer(
            noising_matrix=strategy_inverse_fn(),
            stddev=1.0,
            out_sharding=out_sharding if provide_out_sharding else None,
            internal_flat_shardings=internal_flat_shardings,
            mesh=None if provide_out_sharding else mesh,
        )
    )

    # Note: init will not produce correct shardings unless called from the same
    # jitted context as privatizer.privatize.
    @jax.jit
    ...
    flat_noise = np.concatenate(
        [x.flatten() for x in jax.tree.flatten(noisy_grads)[0]] +
        [x.flatten() for x in jax.tree.flatten(noisy_grads2)[0]]
    )
  ...
  def test_internal_shardings(self):
    ...
    for state in foo(jnp.zeros((3, 4, 5))):
