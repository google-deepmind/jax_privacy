...
from jax_privacy.dpftrl_mechanisms import optimization
from jax_privacy.dpftrl_mechanisms import sensitivity
...
class LossFn:
  ...
  def compute_penalties(
    ...
    These correspond to the conditions of Theorem 1 of "On the Inverse of
    Buffered Linear Toeplitz (BLT) Matricesand Applications to Streaming
    ...
    # Optimizing for multiple participations can produce theta >= 1, which
    # could be a feature, but our sensitivity calculations require
    # non-increasing toeplitz coefficients, which a theta > 1 can break.
    # Using a hard penalty like require_buf_decay_less_eq_one
    # broke the optimization (probably due to gradients at theta=1 calling
    # for an increased theta), but this penalty seems to resolve the issue:
    penalties['buf_decay<1'] = _lt_one_penalty(blt.buf_decay)
    ...
    # we know the optimal 2nd coefficient is 0.5, so we may use
  ...
  def penalized_loss(
    ...
    weighted_penalties = jnp.array(
        [v * multipliers.pop(k, 1.0) for k, v in penalty_dict.items()]
    )
    total_penalty = self.penalty_strength * jnp.sum(weighted_penalties)
    if multipliers:
      raise ValueError(
...
def get_init_blt(
    ...
    largest_theta: float = 0.9,
    smallest_theta: float = 0.1,
) -> BufferedToeplitz:
  """Returns an initial BufferedToeplitz for initializing optimization."""
  if largest_theta >= 1:
    raise ValueError(f'largest_theta={largest_theta=} must be < 1, but is not.')
  # Note: The defaults have not been carefully tuned, but seem to work well
  # enough for optimizing the multiple-participations case. Note that even
  # though typical solutions have a largest_theta values like 0.99999, using
  # 0.9 works better for initializing LBFGS.
  if not init_blt:
    theta = jnp.linspace(largest_theta, smallest_theta, num=num_buffers)
    omega = 0.5 * jnp.ones(num_buffers) / num_buffers
    init_blt = BufferedToeplitz.build(output_scale=omega, buf_decay=theta)
...
def optimize_loss(
  ...
  if parameterization is None:
    parameterization = Parameterization.strategy_blt()
...
def blt_pair_from_theta_pair(
  ...
  used in the implementation here appears in the upcoming survey "Correlated
  Noise Mechanisms for Differentially Private Learning: A Tutorial on
  Theoretical Principles and Practical Aspects".
