...
from jax_privacy.dpftrl_mechanisms import test_utils
import numpy as np
...
class DenseTest(parameterized.TestCase):
  ...
  def check_sensitivity(self, X, k):
    """Checks that under all epoch participation patterns, sensitivity == 1."""
    n = X.shape[0]
    contrib_matrix = contrib_matrix_builders.epoch_participation_matrix(n, k)
    mx, mn = contrib_matrix_builders.max_min_sensitivity_squared_for_x(
        X, contrib_matrix
    )
    self.assertAlmostEqual(mx, 1, places=6)
    self.assertAlmostEqual(mn, 1, places=6)
  ...
  def test_optimization_worked(self, epochs, bands, equal_norm):
    ...
    self.check_psd(X)
    self.check_sensitivity(X, epochs)
