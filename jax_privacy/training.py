...
def _get_batch(
  ...
  if _compilation.is_map_dataset(dataset):
    ...
    elements = [None if i == -1 else dataset[i] for i in indices]
    template = next((e for e in elements if e is not None), None)
    if template is None:
      template = dataset[0]
    elements = [template if e is None else e for e in elements] or [template]
