...
different failure behavior, this can create a distinguishing event and break
the theoretical guarantees. This is unlikely to create meaningful additional
leakage in most deployments, but it remains a formal correctness issue.
...
**How JAX Privacy handles it.** The
{func}`~jax_privacy.clipping.clipped_grad` and
{func}`~jax_privacy.clipping.clipped_fun` functions support empty and padded
...
private record and insert them back into the dataset as separate examples, or
to mix multiple private records into one example, as in Mixup or CutMix.
...
per-record loss, then use {func}`~jax_privacy.clipping.clipped_grad` to clip once
...
Canaries need not be literally independent if the experimental design provides
a defensible exchangeability argument and the statistical bound matches that
...
exchangeable; the library cannot detect source, preprocessing, or selection
bias from the score arrays alone.
...
auditing. The
{meth}`~jax_privacy.auditing.CanaryScoreAuditor.epsilon_one_run` and
{meth}`~jax_privacy.auditing.CanaryScoreAuditor.epsilon_one_run_fdp` methods are
designed for randomized multi-canary, single-training-run audits and account for
the shared-run design. JAX Privacy does not provide a multi-run auditing
...
**The pitfall.** A threshold converts a continuous canary score into a membership
attack: scores on one side are predicted to be members and scores on the other
side are predicted to be non-members. If you try many thresholds and report the
one producing the largest lower bound on the same data, ordinary random
fluctuations are selected along with genuine attack signal. The nominal
...
- {class}`~jax_privacy.auditing.Bonferroni`, the default, corrects for searching
  all candidate thresholds. It is simple and robust, but can be conservative
  when many thresholds are considered.
- {class}`~jax_privacy.auditing.Split` uses one random partition to select the
  threshold and a disjoint partition to evaluate the bound. This avoids reusing
  the same observations for selection and evaluation, at the cost of less data
  in each stage.
- {class}`~jax_privacy.auditing.MultiSplit` repeats the random split, evaluates
  each split at significance level $\alpha/2$, and reports the median lower
  bound. Repetition reduces dependence on one fortunate or unfortunate split,
  but it is still a controlled sample-splitting procedure rather than free
  reuse of the data.
...
- S. De, L. Berrada, J. Hayes, S. L. Smith, and B. Balle.
  ...
  Scale*](https://arxiv.org/abs/2204.13650). 2022. Introduces augmentation
  multiplicity, which averages gradients across several augmentations of one
